...
namespace OregoFramework.Util
{
    public static class EnumerableUtils
    {
        ...
        public static T FindMin<T>(this IEnumerable<T> enumerable, Func<T, IComparable> func)
        {
            using (var enumerator = enumerable.GetEnumerator())
            {
                ...
                var minValue = func.Invoke(minItem);
                while (enumerator.MoveNext())
                {
                    var currentItem = enumerator.Current;
                    var value = func.Invoke(currentItem);
                    if (value.CompareTo(minValue) < Int.ZERO)
                    {
                        minValue = value;
                        minItem = currentItem;
                    }
